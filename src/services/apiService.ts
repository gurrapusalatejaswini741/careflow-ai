...
export const api={
 ...
 analyzeImage:async(file:File):Promise<ImageAnalysisResult>=>{const f=new FormData();f.append('image',file);const r=await fetch(`${API}/api/image-analyze`,{method:'POST',body:f});const data=await r.json().catch(()=>({}));if(!r.ok)throw new Error(data.error||`Request failed: ${r.status}`);return data;}
};
